-- Migration: 0030_lead_generation_crm
-- Full lead capture, pipeline, scoring, and CRM-lite for enterprise content lead-gen.
-- Feeds from workflow triggers and actions defined in 0029_workflow_orchestrator.

-- ─── LEAD CAPTURE FORMS ───────────────────────────────────────────────────────
-- Embeddable forms attached to articles/landing-pages. Each form can trigger a workflow.
CREATE TABLE IF NOT EXISTS lead_capture_forms (
  ...
  show_after_words INTEGER,  -- show after reader has read N words

  -- Workflow to trigger on submission
  workflow_id     TEXT REFERENCES workflows(id) ON DELETE SET NULL,

  -- Fields defined as JSON schema (stored in D1 for simplicity, or KV for large schemas)
  fields_schema   TEXT NOT NULL DEFAULT '[]',  -- JSON array of field definitions
  -- e.g. [{"id":"email","type":"email","label":"Email","required":true},
...
CREATE TABLE IF NOT EXISTS leads (
  ...
  -- Extra dynamic fields stored as JSON (varies per form)
  ...
  -- If the lead is also a Zenos user
  ...
  consent_text    TEXT,         -- the exact consent string shown at capture time
  consent_at      TEXT,
...
CREATE TABLE IF NOT EXISTS lead_tags (
  ...
  tag        TEXT NOT NULL,
  added_by   TEXT REFERENCES users(id) ON DELETE SET NULL,   -- NULL = added by workflow
...
CREATE TABLE IF NOT EXISTS lead_events (
  ...
  -- e.g. 'form_submission','email_opened','email_clicked','article_read',
  --       'score_changed','stage_changed','crm_synced','workflow_triggered',
  --       'manual_note','unsubscribed'
  metadata   TEXT NOT NULL DEFAULT '{}',  -- JSON: event-specific context
  actor_id   TEXT REFERENCES users(id) ON DELETE SET NULL,  -- NULL = system/workflow
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
);
...
-- ─── LEAD SCORE RULES ─────────────────────────────────────────────────────────
-- Rules that automatically adjust a lead's score when events occur.
CREATE TABLE IF NOT EXISTS lead_score_rules (
  id           TEXT PRIMARY KEY,
  org_id       TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  name         TEXT NOT NULL,
  trigger_event TEXT NOT NULL,  -- event_type that fires this rule
  condition    TEXT NOT NULL DEFAULT '{}',  -- JSON: optional filter on event metadata
  score_delta  INTEGER NOT NULL,  -- positive or negative
  is_active    INTEGER NOT NULL DEFAULT 1,
  created_at   TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE TABLE IF NOT EXISTS lead_pipeline_entries (
  ...
  deal_value   INTEGER,           -- estimated value in cents
  notes        TEXT,
...
CREATE TABLE IF NOT EXISTS email_sequences (
  ...
  name          TEXT NOT NULL,
  description   TEXT,
  trigger_type  TEXT NOT NULL DEFAULT 'manual',  -- 'manual','workflow','form_submission'
...
INSERT INTO _migrations (filename) VALUES ('0030_lead_generation_crm.sql');
