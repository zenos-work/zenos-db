-- Migration: 0028_enterprise_orgs_teams
-- Adds enterprise multi-tenancy: organizations, teams, members, invitations, SSO, API keys.
-- Required for: enterprise lead-gen orchestrator, team publishing workflows, org-scoped billing.
...
CREATE TABLE IF NOT EXISTS organizations (
  ...
  -- Settings stored as JSON blob (theme, feature flags, custom domain)
...
-- org_role is separate from the global "role" on users.
...
-- ─── API KEYS (for enterprise integrations / workflow webhooks) ──────────────
CREATE TABLE IF NOT EXISTS api_keys (
  id           TEXT PRIMARY KEY,
  org_id       TEXT REFERENCES organizations(id) ON DELETE CASCADE,
  user_id      TEXT REFERENCES users(id) ON DELETE CASCADE,  -- personal token if org_id is NULL
  name         TEXT NOT NULL,
  key_hash     TEXT NOT NULL UNIQUE,   -- bcrypt / SHA-256 hash; never store plain
  key_prefix   TEXT NOT NULL,          -- first 8 chars shown in UI e.g. "zen_abc1"
...
-- ─── SSO CONFIGURATIONS (future: SAML / OIDC) ────────────────────────────────
CREATE TABLE IF NOT EXISTS sso_configs (
  ...
  metadata      TEXT NOT NULL DEFAULT '{}',  -- JSON: entity_id, sso_url, certificate, etc.
...
-- ─── AUDIT LOG ────────────────────────────────────────────────────────────────
-- Append-only log for security-sensitive actions. No UPDATE/DELETE.
CREATE TABLE IF NOT EXISTS audit_log (
  id          TEXT PRIMARY KEY,
  org_id      TEXT REFERENCES organizations(id) ON DELETE SET NULL,
  actor_id    TEXT REFERENCES users(id) ON DELETE SET NULL,
  actor_ip    TEXT,
  action      TEXT NOT NULL,       -- e.g. 'article.publish', 'workflow.execute', 'member.remove'
  resource    TEXT,                -- resource type  e.g. 'article'
  resource_id TEXT,                -- resource UUID
  payload     TEXT NOT NULL DEFAULT '{}',  -- JSON diff or context
  created_at  TEXT NOT NULL DEFAULT (datetime('now'))
);
...
-- ─── Link existing articles/series to org (optional backfill column) ─────────
ALTER TABLE articles ADD COLUMN org_id TEXT REFERENCES organizations(id) ON DELETE SET NULL;
ALTER TABLE series   ADD COLUMN org_id TEXT REFERENCES organizations(id) ON DELETE SET NULL;
...
INSERT INTO _migrations (filename) VALUES ('0028_enterprise_orgs_teams.sql');
