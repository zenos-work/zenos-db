...
-- ─── GENAI WORKFLOW NODE TYPES ────────────────────────────────────────────────
INSERT OR IGNORE INTO workflow_node_types (id, category, name, description, is_enterprise, config_schema, output_schema) VALUES
  ('transform.genai_summarize',       'transform', 'GenAI: Summarize',           'Summarize long text using LLM (configurable length and style).',                     1, '{"properties":{"provider":{"type":"string","enum":["openai","anthropic","cloudflare_ai"],"default":"openai"},"model":{"type":"string","default":"gpt-4o-mini"},"max_length":{"type":"integer","default":300},"style":{"type":"string","enum":["concise","detailed","bullet_points"],"default":"concise"}}}', '{"properties":{"summary":{"type":"string"},"input_tokens":{"type":"integer"},"output_tokens":{"type":"integer"},"cost_microcents":{"type":"integer"}}}'),
  ('transform.genai_translate',       'transform', 'GenAI: Translate',           'Translate text to target language using LLM.',                                       1, '{"properties":{"provider":{"type":"string","enum":["openai","deepl","cloudflare_ai"],"default":"openai"},"target_language":{"type":"string"},"preserve_formatting":{"type":"boolean","default":true}}}', '{"properties":{"translated_text":{"type":"string"},"source_language":{"type":"string"},"input_tokens":{"type":"integer"},"output_tokens":{"type":"integer"},"cost_microcents":{"type":"integer"}}}'),
  ('transform.genai_rewrite_tone',    'transform', 'GenAI: Rewrite Tone',        'Rewrite text in a different tone (formal, casual, technical, etc.).',                1, '{"properties":{"provider":{"type":"string","enum":["openai","anthropic"],"default":"openai"},"tone":{"type":"string","enum":["formal","casual","technical","friendly","authoritative"],"default":"formal"}}}', '{"properties":{"rewritten_text":{"type":"string"},"input_tokens":{"type":"integer"},"output_tokens":{"type":"integer"},"cost_microcents":{"type":"integer"}}}'),
  ('transform.genai_extract_entities','transform', 'GenAI: Extract Entities',    'Extract named entities (people, organizations, dates, locations) from text.',        1, '{"properties":{"provider":{"type":"string","enum":["openai","cloudflare_ai"],"default":"openai"},"entity_types":{"type":"array","default":["person","org","date","location"]}}}', '{"properties":{"entities":{"type":"array"},"input_tokens":{"type":"integer"},"cost_microcents":{"type":"integer"}}}'),
  ('transform.genai_generate_tags',   'transform', 'GenAI: Generate Tags',       'Auto-generate suggested tags from article content.',                                 1, '{"properties":{"provider":{"type":"string","enum":["openai","cloudflare_ai"],"default":"openai"},"max_tags":{"type":"integer","default":10},"match_existing":{"type":"boolean","default":true}}}', '{"properties":{"tags":{"type":"array"},"input_tokens":{"type":"integer"},"cost_microcents":{"type":"integer"}}}'),
  ('transform.genai_content_moderate','transform', 'GenAI: Content Moderate',    'Run AI safety and content moderation scoring on text.',                              1, '{"properties":{"provider":{"type":"string","enum":["openai","custom_api"],"default":"openai"},"categories":{"type":"array","default":["hate","violence","sexual","self_harm","spam"]}}}', '{"properties":{"safety_score":{"type":"number"},"flags":{"type":"array"},"is_safe":{"type":"boolean"},"input_tokens":{"type":"integer"},"cost_microcents":{"type":"integer"}}}'),
  ('transform.genai_seo_optimize',    'transform', 'GenAI: SEO Optimize',        'Generate SEO suggestions, meta description, and title variants.',                    1, '{"properties":{"provider":{"type":"string","enum":["openai"],"default":"openai"},"target_keyword":{"type":"string"},"max_suggestions":{"type":"integer","default":5}}}', '{"properties":{"seo_title":{"type":"string"},"seo_description":{"type":"string"},"keyword_density":{"type":"number"},"suggestions":{"type":"array"},"input_tokens":{"type":"integer"},"cost_microcents":{"type":"integer"}}}'),
  ('transform.genai_repurpose',       'transform', 'GenAI: Repurpose Content',   'Transform article into tweet thread, LinkedIn post, email summary, or podcast script.', 1, '{"properties":{"provider":{"type":"string","enum":["openai","anthropic"],"default":"openai"},"format":{"type":"string","enum":["tweet_thread","linkedin_post","email_summary","podcast_script","youtube_script"],"default":"tweet_thread"},"max_length":{"type":"integer","default":1000}}}', '{"properties":{"repurposed_content":{"type":"string"},"format":{"type":"string"},"input_tokens":{"type":"integer"},"output_tokens":{"type":"integer"},"cost_microcents":{"type":"integer"}}}');
